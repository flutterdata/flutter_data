...
abstract class LocalAdapter<T extends DataModelMixin<T>> with _Lifecycle {
  ...
  T initModel(T model, {String? key, Function(T)? onModelInitialized}) {
    ...
    if (model._key == null) {
      ...
      if (model.id == null) {
        // use key if supplied
        model._key = key ?? DataHelpers.generateTempKey(internalType);
      } else {
        // (3) if model != null, ignore supplied key
        model._key = DataHelpers.fastHashId(internalType, model.id!);
      }
