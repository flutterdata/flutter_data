...
abstract class DataModel<T extends DataModel<T>> {
  ...
  T _initialize(final Map<String, RemoteAdapter> adapters, {String? key, bool save = false}) {
    ...
    _key = remoteAdapter.graph.getKeyForId(
      remoteAdapter.internalType,
      id,
      keyIfAbsent: key ?? DataHelpers.generateKeyFromClass<T>(),
    );
    ...
    for (final metadata in remoteAdapter.localAdapter
        .relationshipsFor(
          this as T,
        )
        .entries) {
