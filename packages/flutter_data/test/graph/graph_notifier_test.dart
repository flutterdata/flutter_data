...
void main() async {
  ...
  test('add/remove edges with metadata', () {
    ...
    graph.addEdges('h1', tos: ['b1', 'b2'], metadata: 'blogs', inverseMetadata: 'host');
  ...
  test('produces a new key', () {
    ...
    expect(key, isNull);
    key = graph.getKeyForId('people', '1', keyIfAbsent: DataHelpers.generateKeyFromClass<Person>());
    expect(key, startsWith('people#'));
  });

  test('produces a key for empty string', () {
    final key = graph.getKeyForId('people', '', keyIfAbsent: DataHelpers.generateKeyFromClass<Person>());
    expect(key, startsWith('people#'));
  });

  test('deletes a new key', () {
    final key = graph.getKeyForId('people', '1', keyIfAbsent: DataHelpers.generateKeyFromClass<Person>())!;
  ...
  test('does not associate a key when id is null', () {
    var key = graph.getKeyForId('people', null, keyIfAbsent: DataHelpers.generateKeyFromClass<Person>())!;
    expect(graph.getIdForKey(key), isNull);
  });
  ...
  test('by keys', () {
    ...
    final ids = ['people#a1a1a1', 'people#b2b2b2', 'people#c3c3c3'].map(graph.getIdForKey);
    expect(ids, ['p#1', '2', '3']);
  });
  ...
  test('should prioritize ID', () {
    final key = graph.getKeyForId('people', '772', keyIfAbsent: DataHelpers.generateKeyFromClass<Person>());

    final randomNewKey = DataHelpers.generateKeyFromClass<Person>();

    // we are telling manager to reuse the existing key
    // BUT a key for id=772 already exists, so that one will precede
    final finalKey = graph.getKeyForId('people', '772', keyIfAbsent: randomNewKey);
  ...
  test('keys and IDs do not clash', () {
    ...
    expect(
        graph.toMap().keys.toSet(), containsAll({'people#a2a2a2', 'people#a1a1a1', 'id:people#a1a1a1', 'id:people#1'}));
  ...
  test('saves key', () async {
    ...
    for (var i = 0; i < length; i++) {
      ...
      // add some people
      if (i % div == 0) {
        family.persons!.add(Person(name: 'new kid #$i', age: i).init(container.read));
      }
    ...
    expect(graph.toMap().keys.where((k) => k.startsWith('families')), hasLength(length));
  });
  ...
  test('namespaced keys crud', () {
    ...
    expect(() => graph.addEdge('superman:1', 'nonamespace', metadata: 'nonamespace'), throwsA(isA<AssertionError>()));

    graph.addEdge('superman:1', 'nonamespace', metadata: 'superman:prefix');
    expect(graph.getEdge('superman:1', metadata: 'superman:prefix'), containsAll(['nonamespace']));
  ...
  test('namespace', () {
    expect(StringUtils.namespace('id', StringUtils.typify('posts', 'a9')), 'id:posts#a9');
    expect(StringUtils.namespace('zzz', StringUtils.typify('animals', '278#12')), 'zzz:animals#278#12');
  });
  ...
  test('event', () {
    final event = DataGraphEvent(keys: ['a', 'b'], type: DataGraphEventType.addEdge);
